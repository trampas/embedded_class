...
bool enableSercomNVICAndClocks(Sercom *ptrHw, SercomISR isrHandler, void *ptrCallbackParam, uint8_t nvic_priortiy)
{
	uint32_t gClkId;
	uint32_t num;
	ASSERT(ptrHw);
...
#if defined(__SAMD51__) || defined(__SAME51__)
	uint32_t slowClock;
	uint32_t isr_priority=nvic_priortiy;
...
{
	if (Callbacks[6] != nullptr)
	{
		Callbacks[6](CallbackParameters[6]);
	}
}
...
{
	if (Callbacks[7] != nullptr)
	{
		Callbacks[7](CallbackParameters[7]);
	}
}
