...
void __attribute__((aligned (8))) HardFault_DUMMY(void)
{
    __asm volatile
    (
        " tst lr, #4                                                \n"
...
extern uint32_t __eNVM;
extern uint32_t __sNVM;
...
uint32_t getMaxHeap(void); 
uint32_t getLastFlashAddr(void);

uint32_t getLastFlashAddr(void)
{
	size_t ret =(uint32_t) &__etext;
	ret+=(uint32_t)&__data_end__ - (uint32_t) &__data_start__;
	return ret;
}
...
uint32_t getStackUsed(void)
{
	...
	uint32_t addr=(getMaxHeap()+8); //get double word address
//	
//	if (addr<startofStack){
//		addr=startofStack;
//	}
//	
	ptr = (uint32_t *) ((uint32_t) addr & 0xFFFFFFF0UL); //make lower 8 bits zero.
	
	
	while ((uint32_t) ptr < (uint32_t)&__StackTop)
	{
		if (*ptr != 0xDEADBEEF)
		{
			break;
